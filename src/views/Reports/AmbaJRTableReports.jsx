...
class AmbassadorTableReports extends React.Component {
  ...
  render() {
    ...
    const question = report_ambassador.JR.map((prop)=>{
      let TableData=[]
      return(
        TableData=[[t(prop.question)]]
      );
    });
    const studentPre  = report_ambassador.JR.map((prop)=>{
      let TableData=[]
      return(
        TableData=[prop.studentsPre]
      );
    });
    const students  = report_ambassador.JR.map((prop)=>{
      let TableData=[]
      return(
        TableData=[prop.students]
      );
    });
    const percentagePre  = report_ambassador.JR.map((prop)=>{
      let TableData=[]
      return(
        TableData=[prop.percentagePre +"%"]
      );
    });
    const percentage = report_ambassador.JR.map((prop)=>{
      let TableData=[]
      return(
